...
export default function Productcard(prop: Props) {
    ...
    const buyHashbot = async () => {
        ...
        try {
            ...
            const response = await toast.promise(request, {
                ...
                success: `You successfully purchased ${prop.name}`,
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Allow deletion and empty state
        if (inputValue === '') {
            setVal('');  // keep empty
            return;
        }
        ...
        // Set value only if it's within the range
        if (Number(numericValue) <= prop.max) {
            setVal(numericValue);
        }
    }
            ...
            <div className=' relative w-full flex flex-col bg-zinc-900 rounded-sm p-6 h-auto'>
                ...
                <div className=' w-full flex flex-col gap-2 mt-4'>
                    ...
                    <div className=' w-full flex md:flex-row flex-col gap-6 md:items-center justify-between mt-2'>
                        <p className=' text-sm font-semibold'>Selected price : <span className=' text-orange-300'>Php {Number(val).toLocaleString()}</span></p>
