...
import { superadmin, user } from '@/types/routes'
  ...
  return (
      <div className=" min-h-screen w-full flex flex-col overflow-hidden">
        ...
        <div className=" relative h-screen flex flex-col items-center overflow-y-auto">
          <header className=" max-w-[1520px] w-full sticky top-0 z-50 flex h-14 border-b-[1px] border-zinc-800 bg-zinc-950 items-center justify-between gap-4 bg-secondary p-4 lg:h-[74px] lg:px-6">
              ...
              <DropdownMenuTrigger className=' active:border-none focus:border-none'>
                <div className=' flex items-center gap-2'>
                  <div className=' flex flex-col'>
                    <p className=' text-xs text-white'>Superadmin</p>
                  </div>
          ...
          <div className=" bg-zinc-950 z-50 sticky top-[74px] max-w-[1520px] w-full hidden lg:block py-2 border-b-[1px] border-zinc-800">
           <nav className=" flex items-center gap-4 px-2 text-sm font-medium lg:px-4 py-2">
            ...
            {superadmin.map((item, index) => (
              ...
              key={index}
                href={item.route}
                className={` ${path.includes(item.route) ? ' bg-gradient' : 'text-zinc-100'}  text-xs flex items-center gap-1 rounded-sm px-3  py-2 hover:bg-gradient-to-r from-yellow-800 to-yellow-500`}
              >
                {item.icon}
                {item.name}
              </Link>
