...
const add = async (data: AddSocialMedia) => {
    setLoading(true);
    try {
        const request = axios.post(`${process.env.NEXT_PUBLIC_URL}/sociallinks/createsociallink`, {
            title: data.type,
            link: data.link
        }, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger className=" text-xs flex items-center gap-1 bg-yellow-500 py-1 px-3 text-white rounded-sm">
        <Plus size={15}/>Add
      </DialogTrigger>
