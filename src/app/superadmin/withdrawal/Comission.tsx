...
export default function Comissionlist() {
  ...
  return (
    <div className=' relative w-full flex flex-col items-center gap-8 max-w-[1740px] min-h-[500px] h-auto mt-12 bg-zinc-900 p-6'>
        <div className=' flex md:flex-row flex-col items-center justify-between absolute top-0 w-[98%] z-30 gap-2 h-auto md:h-[55px] bg-gradient-to-r from-yellow-700 to-yellow-500 p-2 rounded-sm -translate-y-4'>
            <p className=' text-sm font-semibold'>Commission List</p>

            <div className=' flex flex-wrap items-center gap-2'>
              <Select value={payemnt} onValueChange={setPayment}>
                <SelectTrigger className="w-[150px] bg-zinc-900 text-white">
              ...
              <input value={date} onChange={(e) => setDate(e.target.value)} type="date" className=' text-xs p-2 text-black rounded-sm' />
                ...
                <button onClick={reset} className=' p-2 bg-yellow-700 rounded-sm'><RefreshCcw size={15}/></button>
            </div>
                ...
                <TableCell className="font-medium text-center flex items-center gap-2">
                  ...
                  <Dialog>
                    ...
                    <DialogContent>
                      ...
                      <Button onClick={() => proccessPayout(item.id,item.username)} className=' btn-gradient rounded-sm w-fit text-sm bg-yellow-600 text-white font-semibold flex items-center justify-center gap-2'>
                        {loading2 === true && (
                          <Spinner/>
                        )}
                        Continue</Button>
