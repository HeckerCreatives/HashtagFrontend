...
import { payin, Payin } from '@/validations/schema';
    ...
    <div className=' max-w-[400px] h-fit p-6 w-full bg-zinc-900 rounded-sm flex flex-col'>
        ...
        <form onSubmit={handleSubmit(payinUser)} action="" className=' relative z-30 w-full flex flex-col gap-1 text-sm'>
          ...
          />
            ...
            {open === true && (
              <div className=' absolute w-full flex flex-col p-6 gap-1 h-[300px] bg-zinc-800 overflow-y-auto'>
                ...
                {list.map((item, index) => (
                  ...
                  className=' p-2 w-full bg-zinc-950 cursor-pointer'>{item.username}</p>
                ))}
              </div>
            )}
